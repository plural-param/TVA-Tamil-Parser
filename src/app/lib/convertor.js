...
export function tamil_convertor(text) {
    text = convert_tam_wx2utf(text);
    return text.replace('^', '');
}

export function convertMorph(text) {
    let words = text.split("<");
    let root = tamil_convertor(words[0]);
    return root + "<" + words.slice(1).join("<");
}
